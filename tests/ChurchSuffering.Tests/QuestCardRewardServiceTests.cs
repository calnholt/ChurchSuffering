...
public class QuestCardRewardServiceTests
{
	...
	public void ApplyPendingOfferOption_upgrade_persists_upgraded_key_without_stat_mutation()
	{
		...
		Assert.Equal(1, SaveCache.GetAcceptedDeckRewardMutationCount());
		var baseCard = ChurchSuffering.ECS.Factories.CardFactory.Create("smite");
		var upgradedCard = ChurchSuffering.ECS.Factories.CardFactory.Create("smite");
	...
	public void CardKeyParser_accepts_legacy_and_upgraded_keys()
	{
		...
		Assert.Equal(ChurchSuffering.ECS.Components.CardData.CardColor.White, legacyColor);
		...
		Assert.Equal(ChurchSuffering.ECS.Components.CardData.CardColor.Red, upgradedColor);
