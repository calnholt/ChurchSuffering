...
/// <summary>
/// Dual-status fixture for Poison + Scorched composition on a shared padded surface.
/// </summary>
public sealed class PoisonScorchedCardSnapshotFixture : IDisplaySnapshotFixture
{
    ...
    public string Id => "poison-scorched-card";
    ...
    public void Setup(DisplaySnapshotContext ctx, string[] args)
    {
        ...
        Console.WriteLine($"[DisplaySnapshot] Rendering poison+scorched card: {_cardId}");
    }
    ...
    private static string ParseCardId(string[] args)
    {
        ...
        foreach (string arg in args ?? Array.Empty<string>())
        {
            ...
            {
                throw new DisplaySnapshotSetupException($"Unknown poison-scorched-card option: '{arg}'");
            }
            if (cardIdSet)
            {
                throw new DisplaySnapshotSetupException($"Unexpected poison-scorched-card argument: '{arg}'");
            }
